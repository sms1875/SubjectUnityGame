...
public class MemoryPool
{
    ...
    {
        public bool isActive; // "gameObject"의 활성화/비활성화
        public GameObject gameObject; // 화면에 보이는 실제 게임 오브젝트
    }

    private int increaseCount = 100; // 오브젝트가 부족할 때 Instantiate()로 추가 생성되는 오브젝트
    ...
    // 모든 오브젝트가 현재 사용중이면 InstantiateObjects()로 추가 생성
    public GameObject ActivatePoolItem()
    {
        if (poolItemList == null) return null;

        // 현재 생성해서 관리하는 모든 오브젝트 개수와 현재 활성화 상태인 오브젝트 개수 비교
        // 모든 오브젝트가 활성화 상태이면 새 오브젝트 필요
